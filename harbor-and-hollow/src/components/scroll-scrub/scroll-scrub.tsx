/* Scroll scrub runtime: seam-locked camera journey (architecture A). */
...
export interface ScrollScrubScene {
  ...
  actions?: ReactNode;
  align?: "left" | "right";
  /** Scene-aware editorial placement. Keeps type in the photograph's negative space. */
  position?:
    | "left-top"
    | "left-mid"
    | "right-top"
    ...
    | "center-top"
    | "center-bottom";
  /** Dark type on pale scenes, light type on shadowed scenes. */
  tone?: "dark" | "light";
  ...
  useEffect(() => {
    ...
    const loadClip = (segment: RuntimeSegment) => {
      ...
      video.addEventListener("seeked", () => {
        ...
        // A paused video may never invoke requestVideoFrameCallback after a seek.
        // Two paint frames let the decoded image settle before replacing the poster.
    ...
    const readScroll = () => {
      ...
      for (const [index, segment] of runtime.entries()) {
        ...
        segment.layer.style.visibility = segment.visible ? "visible" : "hidden";
        segment.layer.toggleAttribute("data-visible-layer", segment.visible);
      }
      ...
      if (isMobile()) {
        const neighborIndex =
          scrollDirection < 0 || currentProgress < 0.12
            ? currentIndex - 1
            : currentIndex + 1;
        const retained = new Set([currentIndex, neighborIndex]);
    ...
    const updateVideos = (now: number) => {
      for (const segment of runtime) {
        const { video } = segment;
        if (!video || !segment.ready || video.seeking) {
          continue;
        }
        if (!segment.visible || now - segment.lastSeekAt < 42) {
          continue;
        }

        // Native scroll is the clock. Never ease a second time behind it.
        segment.current = segment.target;
        const rawTargetTime =
          clamp(segment.current, 0, 0.999) * (video.duration || 1);
        // The supplied clips are 24 fps; seeking between source frames adds
        // decoder work but no visible detail.
        const targetTime = Math.round(rawTargetTime * 24) / 24;
        const epsilon = 0.018;
        if (Math.abs(video.currentTime - targetTime) > epsilon) {
          try {
            video.currentTime = targetTime;
            segment.lastSeekAt = now;
          } catch {
  ...
  return (
    ...
    >
      <div className="scroll-scrub__stage">
        ...
        <a className="scroll-scrub__credit" href="/">Independent concept by Aether Sites ↗</a>
        ...
        {scenes.map((scene, index) => {
          ...
          return (
            <article
              className="scroll-scrub__chapter scroll-scrub__stage-chapter"
              data-align={scene.align ?? "left"}
              data-position={scene.position ?? (scene.align === "right" ? "right-mid" : "left-mid")}
              data-section-index={index}
              data-tone={scene.tone ?? "dark"}
              key={scene.id}
            >
      ...
      <div className="scroll-scrub__story">
        {segments.map((segment) => {
          ...
          return <div className="scroll-scrub__chapter-band" data-scroll-scrub-band="" id={segment.scene?.id} key={segment.key} style={bandStyle} />;
        })}
        <div aria-hidden="true" className="scroll-scrub__exit" />
      </div>
    </section>
  );
}
